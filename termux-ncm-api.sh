...
# ---------- 2.5 写入内置脚本3：AI 命令执行服务（cmd-runner.js，端口 3002，工作台 Agent 执行命令/git 用） ----------
write_cmd_runner() {
  step "创建 AI 命令执行服务 $XSH_DIR/cmd-runner.js ..."
  cat > "$XSH_DIR/cmd-runner.js" <<'XSH_RUNNER_EOF'
...
XSH_RUNNER_EOF
  ok "cmd-runner 已就绪: $XSH_DIR/cmd-runner.js"
}
...
pkg install -y git 2>/dev/null || true   # git 工具（github_clone/commit/push 用）
