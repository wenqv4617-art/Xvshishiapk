const CACHE_NAME = 'story-phone-v145';
// v145: ①导出选择器改为**内嵌在「导入/导出」卡片的导出页里**，不再另起浮层——
//       之前那条路踩过两次「面板被档案库/世界书盖住」，因为每个 .app-window 都是
//       各自的层叠上下文；内嵌后只有一个卡片、一套层叠关系，层级问题从根上消失。
//       ②修分组条目「点一下就自己收起」：展开/收起与勾选职责分离，
//       标题区与新增的 ▸/▾ 按钮只管展开，勾选框只管选择；整组勾选后保持展开。
//       ③图片管理补上卡片外框（上一版只给了高度，缩略图之间只剩网格缝隙，
//       外框/圆角/标签底色都看不出来，观感是一大堆图挤在一起）；v1.5.66
...
//       ②一次长上下文生成常要 30~60 秒，超过 45 秒的失联判定阈值，网页明明在正常
//         生成却会被原生抢答 → 网页在生成期间每 5 秒续一次心跳，真正被冻结时
//         心跳自然变旧，原生照旧能接管（同一条消息的原子认领保持不变）；v1.5.63
// v141: 微信 ClawBot 保活加固 + 原生兜底回信 —— ①保活看门狗（AlarmManager 自续期闹钟链）：
//       进程被国产 ROM 清掉后自动把前台服务与长轮询拉回来；②划掉最近任务卡片时主动抢救；
//       ③长轮询期间持短时唤醒锁，Doze 下不再半途断流；④原生侧新增 sendmessage/getconfig/
//       sendtyping，不依赖 WebView 也能发微信；⑤网页每次请求前把拼好的完整上下文交给原生，
//       WebView 被冻结时由原生用同源上下文生成并发出回复（同一条消息有原子认领去重）；
//       ⑥接入面板新增「保活与兜底」诊断卡：常驻服务/收发线程/网页应答端/看门狗/电池白名单
//       五个故障点一目了然，并能一键体检修复、一键加入电池优化白名单；v1.5.62
...
// v138: 修「收消息失败：HTTP 500 / 聊到一半掉线」—— 前后台两个实例同时长轮询同一个 bot token，
//       改为单实例持有（后台中枢优先，前台让出，心跳过期前台无缝接管）；回信延迟下限放开到 0 秒；v1.5.59
