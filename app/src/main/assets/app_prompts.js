...
[PLAY_MUSIC]{"index": 歌曲索引}\n`;
    ...
    mcpPrompt += `\n【核心交互指令二 · 自主设闹钟】：你拥有为用户设定闹钟的能力。当用户明确请求你叫醒、提醒、定时（如"明天叫我起床"、"半小时后提醒我"、"该睡觉了叫我"），或者你基于当前语境判断应该在某个时刻主动提醒用户时，你可以主动为用户设定闹钟。
若你想为用户设定闹钟，请在你的回复文本最末尾追加以下格式的指令（必须单独占一行）：
[SET_ALARM]{"delay": 倒计时秒数, "title": "闹钟留言", "ringtone": 可选-歌曲索引或歌曲标题}
- delay：必填，多少秒后响铃（例如 1800 表示 30 分钟后，3600 表示 1 小时后）
- title：必填，闹钟的留言内容，用自然口语表达（如"该起床啦懒虫"、"半小时到了，去喝水"）
- ringtone：可选，希望闹钟响铃时播放的音乐。可传歌曲索引（数字，对应上面的歌曲索引）或歌曲标题（字符串，会模糊匹配）。不填或留空则用系统默认铃声
- 示例1：[SET_ALARM]{"delay": 1800, "title": "半小时到了，该休息啦", "ringtone": 3}
- 示例2：[SET_ALARM]{"delay": 28800, "title": "早上好！该起床啦", "ringtone": "晴天"}
- 示例3（默认铃声）：[SET_ALARM]{"delay": 600, "title": "十分钟到了"}
注意：设定闹钟后请在正文中自然告知用户（如"好，我给你设了半小时后的闹钟"），不要只甩指令。`;
