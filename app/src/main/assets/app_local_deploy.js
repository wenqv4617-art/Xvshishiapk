...
(function () {
   ...
   * ★ 模块级崩溃防护（"闸门"）
   *
   * 背景：本模块曾经因为在顶层初始化时调用了一个会抛异常的 Base64 解码函数，
   * 导致**整个模块的顶层执行中断** —— window.localDeploySystem 不会被赋值，
   * 于是「本地部署」页面虽然静态骨架渲染出来了，但脚本列表空白、
   * 所有按钮（含「部署引导」）全部点不动，整页等于死掉。
   *
   * 这类「一个数据字段坏了 → 整页陪葬」的失败模式代价太高，所以这里设一道闸门：
   * 任何非期望的异常都会被捕获并显示成一个可见的错误，而不是静默中断模块。
   *
   * 注意：闸门只包住**初始化那一段**（内置脚本构造等），
   * 不包住整个 UI 逻辑 —— 那样反而会把真正的 bug 藏起来、更难查。
   */
  var LOCAL_DEPLOY_INIT_ERROR = "";

  function startupGuard(fn, fallback) {
    try {
      return fn();
    } catch (e) {
      LOCAL_DEPLOY_INIT_ERROR = String((e && e.message) || e);
      try { console.error("[本地部署] 初始化异常，已降级继续:", e); } catch (e2) { }
      return fallback;
    }
  }
...
`;
  ...
  /**
   * 把 Base64 内嵌的源码还原成 UTF-8 字符串。
   *
   * ⚠ 这里曾经用 `decodeURIComponent(escape(atob(x)))` —— 这个「常见技巧」只对
   *   纯 8 位字节安全，遇到不合法的 UTF-8 序列会抛 `URI malformed`。
   *   而本函数是在 **BUILTIN_SCRIPTS 初始化时**就被调用的，一旦抛异常，
   *   整个模块的顶层执行中断 → window.localDeploySystem 根本不会建立 →
   *   真机现象就是「本地部署页整页死掉：脚本列表空白、部署引导按钮点不动」。
   *   正确做法：按字节解出来，再用 TextDecoder 显式按 UTF-8 解码。
   */
  function getLinkMetaSource() {
    try {
      var bin = atob(LINK_META_SOURCE_B64);
      var bytes = new Uint8Array(bin.length);
      for (var i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
      return new TextDecoder("utf-8").decode(bytes);
    } catch (e) {
      // 绝不抛出去：这个函数的返回值只是脚本列表里的一个字段，不值得拖垮整页
      try { console.warn("[本地部署] link-meta 源码解码失败", e); } catch (e2) { }
      return "// link-meta 源码解码失败；可重新安装最新版 APK，或改用「新增脚本」手动填写。";
    }
  }


  var BUILTIN_SCRIPTS = startupGuard(function () {
    return [
    ...
    // 兜底：即使某个脚本源码解码失败，也必须给出一个可用的列表，
    // 让页面其余功能（部署引导、新增脚本）照常工作。
    { id: "ncm-api", name: "网易云音乐 API", desc: "网易云登录代理 / 歌单同步 / 歌词搜索（端口 3000）", port: 3000, healthUrl: "http://localhost:3000/search?keywords=test&limit=1", termuxCmd: "NeteaseCloudMusicApi -p 3000", fileContent: "", isBuiltin: true },
    { id: "cors-proxy", name: "CORS 跨域中转", desc: "为 PWA/网页版打破跨域限制（端口 3001）", port: 3001, healthUrl: "http://localhost:3001/health", termuxCmd: "node $HOME/.xvshishi/cors-proxy.js", fileContent: CORS_PROXY_SOURCE, isBuiltin: true },
    { id: "cmd-runner", name: "AI 命令执行服务（工作台）", desc: "工作台 Agent 执行 termux 命令（端口 3002）", port: 3002, healthUrl: "http://localhost:3002/health", termuxCmd: "node $HOME/.xvshishi/cmd-runner.js", fileContent: CMD_RUNNER_SOURCE, isBuiltin: true }
  ]);

  /**
   * 历史上存在过、但现在已经从代码里删掉的内置脚本 id。
   *
   * 为什么需要这张表：内置脚本是「代码里定义 + 用户 localStorage 里存档」合并出来的。
   * 某个内置脚本从代码里删掉之后，老用户 localStorage 里那份存档仍然存在，
   * 而合并逻辑会把「不在内置列表里的条目」当成用户自定义脚本保留下来 ——
   * 结果就是代码里明明删了，界面上却还一直显示（用户反馈的「接入微信 clawbot 脚本
   * 可以删除了」正是这种幽灵条目）。这里显式把已下线的内置 id 一并清掉。
   *
   * 注意：只清这些明确的 id，不动用户自己新增的脚本。
   */
  var RETIRED_BUILTIN_IDS = [
    "claw", "clawbot", "openclaw", "wechat-claw", "ilink", "wx-ilink"
  ];
  ...
  function loadScripts() {
    ...
    // 先摘掉已下线的内置脚本幽灵条目，并顺手回写一次（清干净，不用等用户手动删）
    var before = saved.length;
    saved = saved.filter(function (s) {
      return !(s && s.id && RETIRED_BUILTIN_IDS.indexOf(String(s.id)) !== -1);
    });
    if (saved.length !== before) {
      try {
        localStorage.setItem("local-deploy-scripts", JSON.stringify(saved));
        console.log("[本地部署] 已清理 " + (before - saved.length) + " 个已下线的内置脚本条目");
      } catch (e) { }
    }
  ...
  var localDeploySystem = {
    ...
    showGuide: function () {
      ...
      var steps = [
        ...
        { title: "第 2 步（推荐）：导出脚本到手机存储", desc: "先点下面的『导出脚本到手机存储』按钮（App 会把 5 个脚本直接写进手机 Download/Storypoem/xvshishi-scripts/，不经过剪贴板，绝不会被截断），然后在 Termux 执行这条短命令安装：", cmd: "mkdir -p ~/.xvshishi \"$PREFIX/bin\" && cp /sdcard/Download/Storypoem/xvshishi-scripts/* ~/.xvshishi/ && cp ~/.xvshishi/xvshishi \"$PREFIX/bin/xvshishi\" && chmod +x ~/.xvshishi/xvshishi-services.sh \"$PREFIX/bin/xvshishi\" && xvshishi" },
