...
> 手册版本: v1.5.52（2026-09-24），已对照更新日志同步到 v4.63。
...
- 注意: 网易云 **VIP 歌曲现在可以整首播放**（会用你的登录态换取带会员鉴权的完整音源，依次尝试极高 / 无损 / 较高 / 标准音质，失败才回落到试听地址）
- 专用 API: API 协议设置里新增「专用」页签，可以给聊天、总结与记忆、查手机、邂逅、深谈、阅读、论坛、情侣空间、一起听歌、购物、快穿局、工作台、**心动游戏**分别指定用哪个预设，没选的自动跟随「链接」里的全局预设，支持一键全部恢复为跟随
- 破限世界书: 现在**所有场景都生效**（主聊天、总结、翻译、心声、朋友圈、查手机、深谈、小剧场、桌宠、工作台），在请求最底层统一注入；每个 API 预设还能各自挂载不同强度的破限世界书，切换预设时跟着切
...
- 它是什么: 把**你手机里真实的微信**接到 App 里。微信来消息会同步进你绑定的那个 char 单聊，char 的回复可以按开关用你自己的账号发回微信去
- 入口在哪: 聊天页面 → 底部「我的」→ 在「应用小助手」的**下一行**就是「微信接入」
- 怎么开（三步）:
  1. 进「微信接入」面板，点「去开启」，跳到系统无障碍设置
  2. 在列表里找到「叙事诗小手机 · 微信接入」并打开
  3. 如果开关被系统拦住（安卓 13 以上对侧载应用有「受限设置」保护），先到 设置 → 应用 → 叙事诗小手机 → 右上角三个点 → 「允许受限设置」，再回来开
- 面板里能配什么:
  - **对接哪个 char**: 面板会列出你当前人设下的全部单聊，点一个就绑定。微信里那个人的昵称要和这个 char 的名字（或备注）**一致**，否则消息会被当成「不在绑定范围内」跳过 —— 这是防串台的保护
  - **接收微信消息**: 总闸，默认关。打开后才会读微信新消息并同步到 App
  - **自动回信到微信**: 默认关。打开后用你自己的账号把 char 的回复发出去。开之前请读面板上的风险提示
  - **自动跳转到目标会话**: 回信时会话没打开时，用微信搜索跳过去；关掉则只在你手动打开该会话后才发
  - **随机延迟 / 频率上限**: 控制回信节奏（默认 3-8 秒延迟、每小时最多 20 条），避免太机器化
  - **运行数据**: 收到 / 生成回复 / 已发出 / 已拦截 的计数，以及原生服务状态、当前微信会话名、待发队列长度
  - **事件流**: 最近发生的事（收到谁的消息、生成了什么、发出去了什么、被什么原因拦下）
  - 下面的按钮：打开微信、立即读一次、清空收件箱、清空待发、清理上下文、计数归零
- 它只做什么、不做什么:
  - **只监听微信**（com.tencent.mm），其它任何 App 的界面内容都不读
  - 打字、点发送走的是**微信自己的客户端流程**
  - **不碰微信数据库**，不伪造消息，不后台静默群发
  - 发送前一定会核对「当前打开的微信会话名 == 目标会话名」，**对不上就不发** —— 宁可不发，也不发错人
  - 同步是**单向**的：微信 → App。你在 App 里发的消息不会自动发到微信，避免两边来回打架
- 关于风险（这里必须说实话）:
  - 自动化发送消息**有被微信风控的可能**。判定的依据在腾讯的服务端，和你是不是本机操作无关 —— 「本机操作所以安全」这个说法是错的
  - 已做的降险措施：默认全部关闭、回信带随机延迟、有每小时上限、发送前复核会话名。但**风险降不到零**，请自己决定用不用、用多勤
- 换了聊天对象怎么办: 在「对接哪个 char」里点另一个 char 就切换绑定了；点「解除绑定」则完全断开
- 上下文与清理: 微信通道自己维护一份短期上下文，**按微信会话名隔离**（不会串台），每个会话最多留 40 条、超过 2 小时无动静就自动丢掉、最多保留 12 个会话；面板上的「清理上下文」可以手动清空。另外所有要发到微信的文本都会先做标签清洗，CoT、心声、卡片指令、HTML 这些都不会漏出去
