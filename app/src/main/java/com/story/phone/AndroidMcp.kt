...
class AndroidMcp private constructor(private val context: Context) {
    ...
    // ============================================================
    //  微信接入（无障碍）：状态 / 读取 / 回信 / 拉起微信
    //  说明：这里只做「桥梁」，真正的界面读取与文字注入在 WeChatAccessibilityService。
    //       所有会往微信写入的动作，都必须在服务侧通过「会话名校验」才会执行。
    // ============================================================

    /** 读取当前已启用的无障碍服务列表（Android 用冒号分隔的扁平串存放） */
    private fun enabledAccessibilityServices(): List<String> {
        return try {
            ...
            flat.split(':').map { it.trim() }.filter { it.isNotEmpty() }
        } catch (e: Exception) {
            emptyList()
        }
    }

    /** 本应用的微信接入无障碍服务是否已在系统设置里被开启 */
    @JavascriptInterface
    fun wechatA11yIsEnabled(): String {
        return try {
            val component = android.content.ComponentName(context, WeChatAccessibilityService::class.java)
            val flat = component.flattenToString()
            val flatShort = component.flattenToShortString()
            val enabled = enabledAccessibilityServices().any {
                it.equals(flat, true) || it.equals(flatShort, true) ||
                    it.endsWith("WeChatAccessibilityService", true)
            }
            JSONObject().apply {
                put("ok", true)
                put("enabled", enabled)
                put("running", WeChatAccessibilityService.isRunning())
                put("component", flat)
                put("secureEnabled", Settings.Secure.getInt(
                    context.contentResolver,
                    Settings.Secure.ACCESSIBILITY_ENABLED, 0
                ) == 1)
            }.toString()
        } catch (e: Exception) {
            "{\"ok\":false,\"enabled\":false,\"running\":false,\"error\":\"" + (e.message ?: "查询失败") + "\"}"
        }
    }

    /** 跳到系统无障碍设置页，由用户手动开启本服务 */
    @JavascriptInterface
    fun wechatA11yOpenSettings() {
        Log.d(TAG, "wechatA11yOpenSettings() called")
        try {
            val intent = Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS).apply {
                flags = Intent.FLAG_ACTIVITY_NEW_TASK
            }
    ...
    /**
     * 下发运行配置。
     * allowedJson 传的是 JSON 数组字符串（会话名白名单），空数组或空串表示不限制。
     */
    @JavascriptInterface
    fun wechatA11yConfigure(
        ...
        return try {
            WeChatAccessibilityService.listenEnabled = listenEnabled
            WeChatAccessibilityService.autoReplyEnabled = autoReplyEnabled
            WeChatAccessibilityService.autoNavigateEnabled = autoNavigateEnabled
            val set = HashSet<String>()
            if (!allowedJson.isNullOrBlank()) {
            ...
            WeChatAccessibilityService.allowedConversations = java.util.Collections.synchronizedSet(set)
            JSONObject().apply {
                put("ok", true)
                put("listenEnabled", listenEnabled)
                put("autoReplyEnabled", autoReplyEnabled)
                put("allowedCount", set.size)
            }.toString()
        } catch (e: Exception) {
            "{\"ok\":false,\"error\":\"" + (e.message ?: "配置失败") + "\"}"
        }
    }

    /** 取走最多 limit 条新消息（取走即出队） */
    @JavascriptInterface
    fun wechatA11yDrainInbox(limit: Int): String {
        return try {
            val arr = WeChatAccessibilityService.drainInbox(limit)
            JSONObject().apply {
                put("ok", true)
                put("messages", arr)
                put("remain", WeChatAccessibilityService.peekInboxSize())
            }.toString()
        } catch (e: Exception) {
            "{\"ok\":false,\"messages\":[],\"error\":\"" + (e.message ?: "读取失败") + "\"}"
        }
    }

    /** 服务运行状态 / 计数 / 当前会话名 / 最近状态说明 */
    @JavascriptInterface
    fun wechatA11yStatus(): String {
        return try {
            WeChatAccessibilityService.buildStatusJson()
        } catch (e: Exception) {
            "{\"ok\":false,\"error\":\"" + (e.message ?: "状态读取失败") + "\"}"
        }
    }

    /** 手动触发一次界面快照（设置页「立即读一次」） */
    @JavascriptInterface
    fun wechatA11yRequestSnapshot(): Boolean {
        return try {
            WeChatAccessibilityService.requestSnapshot()
            true
        } catch (e: Exception) {
            false
        }
    ...
    /** 清空收件箱 */
    @JavascriptInterface
    fun wechatA11yClearInbox(): Boolean {
        return try {
            WeChatAccessibilityService.clearInbox()
            true
        } catch (e: Exception) {
            false
        }
    ...
    /** 清空未发出的回信队列 */
    @JavascriptInterface
    fun wechatA11yClearReplies(): Boolean {
        return try {
            WeChatAccessibilityService.clearReplies()
            true
        } catch (e: Exception) {
            false
        }
    ...
    /** 重置收发计数 */
    @JavascriptInterface
    fun wechatA11yResetStats(): Boolean {
        return try {
            WeChatAccessibilityService.resetStats()
            true
        } catch (e: Exception) {
            false
        }
    ...
    /** 挂起中的回信任务快照 */
    @JavascriptInterface
    fun wechatA11yPendingReplies(): String {
        return try {
            JSONObject().apply {
                put("ok", true)
                put("pending", WeChatAccessibilityService.pendingReplies())
            }.toString()
        } catch (e: Exception) {
            "{\"ok\":false,\"pending\":[],\"error\":\"" + (e.message ?: "读取失败") + "\"}"
        }
    }

    /**
     * 排一条回信任务。
     * 服务只在「当前打开的微信会话名 == conversation」时才会真正发出；
     * autoNavigate 打开时会先尝试用微信搜索跳转过去，跳过去后再复核一次会话名。
     */
    @JavascriptInterface
    fun wechatA11yEnqueueReply(conversation: String, text: String): String {
        return try {
            WeChatAccessibilityService.enqueueReply(conversation, text).toString()
        } catch (e: Exception) {
            "{\"ok\":false,\"error\":\"" + (e.message ?: "入队失败") + "\"}"
        }
    }

    /** 拉起微信（用于用户核对界面的入口按钮） */
    @JavascriptInterface
    fun wechatOpenApp(): Boolean {
        return try {
            val intent = context.packageManager.getLaunchIntentForPackage(WeChatAccessibilityService.WECHAT_PKG)
            if (intent == null) {
                Log.w(TAG, "wechatOpenApp: 未安装微信或无可启动入口")
                false
            } else {
    ...
    /** 微信是否已安装 */
    @JavascriptInterface
    fun wechatIsInstalled(): Boolean {
        return try {
            context.packageManager.getLaunchIntentForPackage(WeChatAccessibilityService.WECHAT_PKG) != null
        } catch (e: Exception) {
            false
        }
    }
